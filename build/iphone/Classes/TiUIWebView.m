...
- (WKWebView *)webView
{
  if (_webView == nil) {
    ...
    [controller addUserScript:[self userScriptXLiteInjectionForAppEvent]];
...
- (WKUserScript *)userScriptXLiteInjectionForAppEvent
{
  if (_pageToken == nil) {
  ...
  NSString *_xlite = [NSString stringWithFormat:@"%@%s", @"Ti", "tanium"];
  ...
  NSString *sourceString = [NSString stringWithFormat:source, _xlite, _pageToken, baseInjectScript];
...
- (WKUserScript *)userScriptXLiteJSEvaluationFromString:(NSString *)string
{
...
- (void)webView:(WKWebView *)webView decidePolicyForNavigationAction:(nonnull WKNavigationAction *)navigationAction decisionHandler:(nonnull void (^)(WKNavigationActionPolicy))decisionHandler
{
  ...
  if ([allowedURLSchemes containsObject:navigationAction.request.URL.scheme]) {
    if ([[UIApplication sharedApplication] canOpenURL:navigationAction.request.URL]) {
      // Event to return url to XLite in order to handle OAuth and more
